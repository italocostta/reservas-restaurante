...
18. **`r.serviceTZ` do `reservation.PostgresRepo` é fixo no boot** (vem do `cfg.ServiceTZ`) e fica *stale* se o staff editar o fuso pelo `PUT /service-hours`. As queries que o usam (`ContarReservasFuturas` da mesa, `ContarReservasNoDia` da exceção, o filtro do `?date=`) passam a ler o dia de parede num fuso que já não é o vigente. A `ContarReservasForaDoExpediente` (seção 16) é a única que evita isso, usando o TZ *candidato* do próprio PUT; as outras não foram migradas. Mudar de fuso é raro e as reservas guardam o instante correto (`timestamptz`), então o efeito é de rótulo, não de corrupção — por isso débito, não bug de corretude.
19. **`DELETE /service-exceptions/{day}` de uma abertura especial re-fecha o dia sem checar reservas.** É a terceira porta do mesmo gap (as duas primeiras — `PUT /service-hours` e `POST /service-exceptions` — foram fechadas nas seções 16 e no item #17). Se o staff abriu com exceção (`is_open=true`) uma segunda-feira normalmente fechada, criou reservas naquele dia, e depois **apaga** a exceção, o dia volta a seguir a regra semanal (fechado) — e as reservas ficam órfãs no dia que a agenda passa a mostrar como fechado. Não foi corrigido: o padrão a replicar é o de sempre (antes de `DeleteExcecao`, se a exceção era `is_open=true`, contar reservas confirmed no dia com `ContarReservasNoDia`, `409` se houver, `500` se falhar). Fica registrado aqui em vez de descoberto por acidente. **Relacionado:** #17, seção 16.
